'use client';
import { IStaffFormValues } from '@/src/app/admin/(admin)/staff/create/StaffRegistrationForm';
/* eslint-disable */
...
interface IDoctor extends IStaffFormValues {
  id: string;
}
const page = () => {
  ...
  const getDepartmentDataForHospital = async () => {
    ...
    const filterDepartments = (departmentValues: string[]) => {
      return DEPARTMENTS.filter(
        (department) => departmentValues?.includes(department.value)
      );
    };
  ...
  const getDoctorsFromDepartment = async () => {
    ...
    const response: any = await http
      .service()
      .get(
        apiRoutes.staff.getStaffByDepartment(
          params.id as string,
  ...
  //   useEffect(() => {
  //     if (active === 1 && form.values.department) {
          ...
          <Stepper.Step label="First step" description="Select a department">
            <div className="bg-primary px-2 py-1 rounded-md flex justify-center xs:mb-3 sm:my-5 lg:my-8">
              <Title order={1} className="  text-textPrimary">
                Select a department {JSON.stringify(active)}
              </Title>
            </div>
              ...
              {departments.map((item: any) => {
                return (
                  <div
                    ...
                    key={item.value}
                    onClick={() => {
                      form.setFieldValue('department', item.value);
                      getDoctorsFromDepartment();
                    }}
                  >
